...
class GraphTestMixin:
    def create_tenant(self):
        return Tenant(tenant='tests{}'.format(uuid4().urn)).save()

    def destroy_tenant_and_data(self, tenant):
        results, meta = db.cypher_query(
            "MATCH (n:Tenant)-[*]->(m) WHERE n.tenant={tenant} DETACH DELETE (m)",
            params={'tenant': tenant.tenant})
        results, meta = db.cypher_query(
            "MATCH (n:Tenant) WHERE n.tenant={tenant} DETACH DELETE (n)",
            params={'tenant': tenant.tenant})

    def create_test_attack_goal(self, tenant):
        ...
        return attack_goal, attacker_strong, attacker_weak


class GraphTestCase(TestCase, GraphTestMixin):
    def setUp(self):
